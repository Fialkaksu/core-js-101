...
function removeLeadingAndTrailingWhitespaces(value) {
  // throw new Error('Not implemented');
  // return value.replace(/(^\s*)|(\s*$)/gi, '').replace(/\s{2,}/gi, ' ');
  return value.trim();
}
...
function getRectangleString(width, height) {
  ...
  // const centralLine = `│${' '.repeat(width - 2)}│\n`;
  // const lastLine = `└${'─'.repeat(width - 2)}┘\n`;
  // const center = `${height <= 2 ? '' : centralLine.repeat(height - 2)}`;

  // return `${firstLine}${center}${lastLine}`;
...
function encodeToRot13(str) {
  ...
  const index = (x) => dec.indexOf(x);
  const translate = (char) => (index(char) > -1 ? enc[index(char)] : char);
